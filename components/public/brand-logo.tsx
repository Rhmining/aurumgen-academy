...
export function BrandLogo({
  ...
  return (
    <span className={clsx("inline-flex items-center gap-3.5 leading-none", className)}>
      {showWordmark ? (
        <span className={clsx("relative block h-12 w-[15rem] shrink-0 md:h-14 md:w-[17rem]", textClassName)}>
          <Image
            src="/brand/aurumgen-academy-logo.png"
            alt="AURUMGEN Academy"
            fill
            priority
            sizes="(min-width: 768px) 272px, 240px"
            className={clsx("object-contain object-left drop-shadow-[0_6px_14px_rgba(240,166,0,0.18)]", markClassName)}
          />
        </span>
      ) : (
        <svg
